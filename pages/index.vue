...
import MainPageTemplate, { MainPageTemplateData } from "~/components/templates/MainPageTemplate.vue";
...
export default class IndexPage extends Vue {
  ...
  private loadTsv(file:File){
    ...
    file.text().then(t => {
      ...
      const sliceNameColIndex = headerline.findIndex(txt => txt.toLowerCase() === 'slicename')
      ...
      labels.forEach(l => {
        const color = colorLine.length > l.index ? colorLine[l.index] : "#808080"
        colors.push({
          label:l.label,
          color
        })
      ...
      lines.slice(2).map(dataLine => dataLine.split('\t')).forEach(line => {
        const title = line.length > sliceNameColIndex ? line[sliceNameColIndex] : ''
